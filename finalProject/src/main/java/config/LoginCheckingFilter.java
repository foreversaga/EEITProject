...
public class LoginCheckingFilter implements Filter {
	...
	public void init(FilterConfig fConfig) throws ServletException {
		urls = fConfig.getServletContext().getFilterRegistration("login").getUrlPatternMappings();
	}
			...
			if (mustLogin()) {           
				if (checkLogin(req)) {   
					...
					if ( ! isRequestedSessionIdValid ) {
						...
						// 記住原本的"servletPath"，稍後如果登入成功，系統可以自動轉入
						// 原本要執行的程式。
						session.setAttribute("requestURI", servletPath);
						session.setAttribute("aContent", req.getParameter("aContent"));
						session.setAttribute("aTitle", req.getParameter("aTitle"));
					}
