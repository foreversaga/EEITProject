...
function CheckOutButton(){
	...
	function ToggleReview(clicked_id) {
		var pId = '"#' + clicked_id + '"';
		var updatepId = '#AmendReview' + clicked_id;
		$("#ShowReview").hide();
		$(updatepId).fadeIn();
		$("#clicked_id").hide();
	};
				...
				<div class="  justify-content-around  " style="text-align: center;">
					<div class="justify-content-center " style="margin-top: 100px">
						...
						<button type="button" class="btn btn-warning btn btn-lg mt-5"
							...
							aria-expanded="false" aria-controls="ProductList">修改商品</button>
					...
					<div class=" sticky-top   "
						...
						<div class="container  p-0 ">
							<div class="container flex-column collapse" id="ProductList">

								<div class="  pt-5">
									<div class="card ">

										<div class="card-header card-header-primary">
											<h4 class="card-title " style="text-align: center">商品管理</h4>
											...
											<div class="container-fruid" style="height: 450px;">
												<div class="">
													<div class="table100 ver1 mb-5">
														<div class="table100-head mt-1 "
															style="text-align: center">
															<table>
																<thead>
																	<tr class="  row100 head justify-content-around"
																		style="text-align: center;">
																		<th class="cell100 column1" style="width: 11%">產品編號</th>
																		<th class="cell100 column2" style="width: 18%">產品名</th>
																		<th class="cell100 column3 ">修改</th>
																		<th class="cell100 column4">庫存</th>
																	</tr>
																</thead>
															</table>
														</div>
																	...
																	<c:forEach varStatus="vs" var="productBean"
																		...
																		<tr class="row100 body" style="font-size: 30px;">
																			<td class="cell100 column1"
																				style="text-align: center; padding-left: 0px;">${productBean.pId}<img
																				width="30%;" src="" /></td>
																			<td class="cell100 column2">${productBean.pName}</td>
																			<td class="cell100 column3"
																				...
																				<button class="btn btn-secondary mt-2" type="button"
																					...
																					data-target="#collapseReview${productBean.pId}"
																					aria-expanded="false"
																					aria-controls="collapseReview${productBean.pId}">
																					查看商品內容</button>
																				<div class="collapse"
																					id="collapseReview${productBean.pId}">
																					<div style="text-align: left; margin-top: 2%;"
																						class="card card-body">
																						<div id="ShowReview">
																							<span class="">地區:${productBean.pArea}</span>
																							<p>分類:${productBean.pType}</p>
																							<p>日期:${productBean.pDateRange}</p>
																							<p>價格:${productBean.pPrice}</p>
																							<p>庫存:${productBean.pInstock}</p>
																							<h5>${productBean.pName}</h5>
																							<p>${productBean.pContent}</p>
																							<button id="${productBean.pId}"
																								onclick="ToggleReview(this.id)"
																								class="btn btn-secondary">修改</button>
																						</div>
																						<div id="AmendReview${productBean.pId}"
																							style="display: none">
																							<!-- 																<div class="card card-body" style="text-align: left; margin-top: 2%;"> -->
																							<form:form
																								action="${pageContext.request.contextPath}/ProductListDashboard"
																								modelAttribute="updateallpb"
																								enctype="multipart/form-data" method="POST">
																								<%--<form:input type="hidden" path="mName" value="${LoginOK.mName}"></form:input> --%>
																								<form:input type="hidden" path="pId"
																									value="${productBean.pId}"></form:input>
																								<form:input type="hidden" path="pId"
																									value="${productBean.pAvgRating}"></form:input>
																								<form:input type="hidden" path="pId"
																									value="${productBean.pDateRange}"></form:input>
																								<form:input type="hidden" path="pId"
																									value="${productBean.pPopular}"></form:input>
																								<form:input type="hidden" path="pId"
																									value="${productBean.pRating}"></form:input>
																								...
																								<form:input class="mt-2"
																									style="margin-right: -300px" type="text"
																									size="20" path="pArea"
																									value="${productBean.pArea}"></form:input>
																								<br>
																								<form:input class="mt-1"
																									style="margin-right: -300px" type="text"
																									size="20" path="pType"
																									value="${productBean.pType}"></form:input>
																								<br>
																								<form:input class="mt-1"
																									style="margin-right: -300px" type="date"
																									size="20" path="pDateRange"
																									value="${productBean.pDateRange}"></form:input>
																								<br>
																								<form:input class="mt-1"
																									style="margin-right: -300px" type="text"
																									size="20" path="pPrice"
																									value="${productBean.pPrice}"></form:input>
																								<br>
																								<form:input class="mt-1"
																									style="margin-right: -300px" type="text"
																									size="20" path="pInstock"
																									value="${productBean.pInstock}"></form:input>
																								<br>
																								<form:input class="mt-1"
																									style="margin-right: -300px" type="text"
																									size="20" path="pLat"
																									value="${productBean.pLat}"></form:input>
																								<br>
																								<form:input class="mt-1"
																									style="margin-right: -300px" type="text"
																									size="20" path="pLng"
																									value="${productBean.pLng}"></form:input>
																								<br>
																								<label class="bmd-label-floating">商品圖檔更新</label>
																								<form:input id="productImage"
																									path="productImage" type="file"
																									class="form-control btn btn-outline-dark btn-block " />
																								<br>
																								<form:input class="mb-1"
																									style="margin-right: -300px" type="text"
																									size="28" path="pName"
																									value="${productBean.pName}"></form:input>
																								<br>
																								<textarea style="margin-right: -220px"
																									name="pContent" rows="8" cols="28">${productBean.pContent}</textarea>
																								<%--<form:input type="hidden" path="pId" value="${reviewBean.pId}"></form:input> --%>
																								<br>
																								<input class="btn btn-outline-success btn-sm"
																									type="submit" value="送出">
																								<button id="${productBean.pId}"
																									onclick="CancelAmend(this.id)"
																									class="btn btn-outline-dark btn-sm">取消</button>
																							</form:form>
																						</div>
																					</div>
																			...
																			<td class="cell100 column4"
																				style="margin-left: 200px; width: 11%">${productBean.pInstock}</td>
																		</tr>
																	</c:forEach>
																</tbody>
															</table>
